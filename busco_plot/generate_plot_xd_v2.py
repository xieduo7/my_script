...
.. versionchanged::xieduo (xieduo@genomics.cn)
...
RCODE = '######################################\n'\
        ...
        '# Output\n'\
        'my_output <- paste(%s1,"busco_figure_%s8.png",sep="/") \n' \
        'my_width <- 20\n' \
        'my_height <- 15\n'\
        'my_unit <- "cm"\n'\
        '\n'\
        ...
        '\n'\
        '# Legend\n'\
        'my_title <- "BUSCO Assessment Results(%s6,n=%s7)"\n'\
        '\n'\
        '# Font\n'\
        'my_family <- "sans"\n'\
        'my_size_ratio <- 1\n'\
        ...
        '\n'\
        'figure <- ggplot(df,aes(y = my_percentage, x = my_species, fill = category)) + \n'\
        '  \n'\
        '  geom_bar(stat="identity",width=my_bar_height,position="dodge") + \n'\
	'  coord_flip() + \n' \
        '  theme_gray(base_size = 8) + \n' \
        '  scale_y_continuous(labels = c("0","20","40","60","80","100"), breaks = c(0,20,40,60,80,100),position = "top") + \n'\
        ...
        '  ylab("\\n%BUSCOs") + \n'\
        '\n'\
        '  theme(plot.title = element_text(hjust = 0.5,family=my_family, colour = "black", size = rel(1.5)*my_size_ratio, face = ' \
        '"bold")) + \n'\
	' theme(plot.margin = margin(1, 1, 1, 1, "cm"))+ \n' \
        '  theme(legend.position="top",legend.title = element_blank()) + \n'\
        '  theme(legend.text = element_text(family=my_family, size = rel(1.2)*my_size_ratio),legend.key.size = unit(0.4, "cm")) + \n'\
	...
	'  theme(axis.ticks.x = element_blank())+\n  '\
        ...
        '  theme(axis.title.x = element_text(family=my_family, size=rel(1.2)*my_size_ratio,hjust = 0)) + \n'\
        '  \n'\
...
def _write_r_code(data):
    ...
    r_file.write(RCODE
		 ...
		 .replace('%s6', data['dataset'])
		 .replace('%s7', data['num'])
                 .replace('%s5', '(1)')
		 .replace('%s8', data['dataset']))
